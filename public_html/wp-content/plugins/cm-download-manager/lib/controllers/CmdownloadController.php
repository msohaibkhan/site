...
//         add_filter('dynamic_sidebar_has_widgets', array(__CLASS__, 'dynamic_sidebar_has_widgets'), 10, 2);
    ...
    {
        if( !empty($_POST['allowed_extensions']) AND self::canSaveSettings())
        {
        ...
        $params['allowed_extensions'] = get_option(CMDM_GroupDownloadPage::ALLOWED_EXTENSIONS_OPTION, self::getDefaultAllowedExtensions());
        return $params;
    }
...
//                     $search .= "{$searchand}(($wpdb->posts.post_title LIKE '{$n}{$term}{$n}') OR ($wpdb->posts.post_content LIKE '{$n}{$term}{$n}'))";
    ...
    public static function overrideTemplate($template)
    {
        if( get_query_var('post_type') == CMDM_GroupDownloadPage::POST_TYPE || is_tax(CMDM_GroupDownloadPage::CAT_TAXONOMY) )
        {
            ...
            if( is_single() || is_404() )
            {
            	wp_enqueue_script('jquery-tools', CMDM_URL . '/views/resources/js/jquery.tools.min.js', array('jquery'));
