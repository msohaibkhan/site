         ...
         */
         ...
         if (textentry_hangman) {
            ...
            if ($edit_obj.hasClass ('qwiz-part1')) {
               new_html = question_text.replace (/[^]*\[(textentry|hangman)/, new_html + textentry_hangman); 
            } else {
               var m = question_text.match (/\[(textentry|hangman)[^\]]*\]/);
               if (m) {
                  textentry_hangman = m[0];
                  new_html = question_text.replace (/\[(textentry|hangman)[^\]]*\][^]*/, textentry_hangman + new_html); 
               }
            }
         }
      ...
      if ($edit_obj.hasClass ('qcard-part1')) {
         ...
         if (i_choice === '') {
            ...
            if (text && text.search (/\[(textentry|hangman)/) != -1) {
               new_html = text.replace (/[^]*\[(textentry|hangman)/, new_html + '[\$1'); 
            }
         }
      } else {
         ...
         if (text) {
            ...
            if (m) {
               ...
               new_html = text.replace (/\[(textentry|hangman)[^\]]*\][^]*/, textentry_hangman + new_html); 
            }
         }
      }
      if (front_b) {
...
function add_style () {
   ...
   s.push ('#qwizard_dataset_question_not_editable,');
   s.push ('#qwizard_dataset_card_not_editable {');
   s.push (   'display:             none;');
   s.push (   'color:               darkgray;');
   s.push (   'padding-left:        1em;');
   s.push ('}');
   ...
   s.push ('.label_options_menu_feedback {');
   s.push (   'display:         none;');
   s.push (   'padding:         5px;');
   s.push (   'background:      #FFFF77;');
   s.push ('}');
   ...
   s.push ('div.qwizard_menu_item_inactive {');
   s.push (   'border:                 1px solid transparent;');
   s.push (   'background:             none;');
...
function quiz_components_html () {
   ...
   if (! new_qwiz_deck_f && ! wp_editing_page_f) {
      ...
      m.push (            '&nbsp;');
      m.push (            '<button class="qbutton qwizard_smaller" style="padding: 1px;">');
      m.push (               'Ok');
      m.push (            '</button>');
      m.push (         '</td>');
      m.push (      '</tr>');
...
function card_shortcodes_text (question, display_b) {
   ...
   if (question.question_text.indexOf ('textentry') != -1) {
      ...
      // ...
      var hangman_attributes = '';
      qhtml = question.question_text.replace (/<div class="qdeck_hangman[^]*<\/div>/, '[hangman' + hangman_attributes + ']');
      var card_front = place_shortcode ('[q' + question_attributes + ']', qhtml);
      card_front = add_paragraph_tags (card_front);
      htm.push (card_front);
   ...
   if (target_must_be_text_f) {
      ...
      var delay_init_selection = function () {
         ...
         var select_text = 'Select text that will be the target "drop zone" for this label'
                           + '<img src="' + qwizzled_params.url + 'images/icon_exit_bw.jpg" class="click_on____exit" onclick="qwizzled.exit_select_text_target ()" />';
